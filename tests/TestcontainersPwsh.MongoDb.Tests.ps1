...
Describe "MongoDbContainer Cmdlets Tests" {
    BeforeAll {
        try {
            Import-Module -Name "$PSScriptRoot\..\src\TestcontainersPwsh.MongoDb\TestcontainersPwsh.MongoDb.psd1" -Force
        } catch {
        ...
        $global:container = New-MongoDbContainer
    ...
    It "Creates a new MongoDbContainer" {
        $global:container | Should -Not -BeNullOrEmpty
    }

    It "Starts MongoDbContainer" {
