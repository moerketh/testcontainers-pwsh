...
Describe "KeycloakContainer Cmdlets Tests" {
    BeforeAll {
        try {
            Import-Module -Name "$PSScriptRoot\..\src\TestcontainersPwsh.Keycloak\TestcontainersPwsh.Keycloak.psd1" -Force
        } catch {
        ...
        $global:container = New-KeycloakContainer
    ...
    It "Creates a new KeycloakContainer" {
        $global:container | Should -Not -BeNullOrEmpty
    }

    It "Starts KeycloakContainer" {
    ...
    It "Gets connection string" {
        $global:container.GetBaseAddress() | Should -Not -BeNullOrEmpty
    }

    It "Stops KeycloakContainer" {
        $global:container | Stop-Container
        Wait-Event Container.Stopped
    }

    It "Removes KeycloakContainer" {
