...
Describe "RedisContainer Cmdlets Tests" {
    BeforeAll {
        try {
            Import-Module -Name "$PSScriptRoot\..\src\TestcontainersPwsh.Redis\TestcontainersPwsh.Redis.psd1" -Force
        } catch {
        ...
        $global:container = New-RedisContainer
    ...
    It "Creates a new RedisContainer" {
        $global:container | Should -Not -BeNullOrEmpty
    }

    It "Starts RedisContainer" {
