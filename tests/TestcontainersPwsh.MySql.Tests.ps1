...
Describe "MySqlContainer Cmdlets Tests" {
    BeforeAll {
        try {
            Import-Module -Name "$PSScriptRoot\..\src\TestcontainersPwsh.MySql\TestcontainersPwsh.MySql.psd1" -Force
        } catch {
        ...
        $global:container = New-MySqlContainer
    ...
    It "Creates a new MySqlContainer" {
        $global:container | Should -Not -BeNullOrEmpty
    }

    It "Starts MySqlContainer" {
